...
class ImageSelectorApp:
    def __init__(self, root):
        ...
        self.label_folder = tk.Label(self.top_frame, text="No folder selected", wraplength=300)
    ...
    def toggle_selection(self, idx):
        """이미지 클릭 시 선택/해제 및 즉시 저장/삭제"""
        if idx < len(self.grid_images):
            img_path = self.grid_images[idx]
            new_state = not self.check_vars[idx].get()
            self.check_vars[idx].set(new_state)
            self.selected_images[img_path] = new_state
            logging.info(f"Toggled selection for image {os.path.basename(img_path)}: {'selected' if new_state else 'deselected'}")
            self.update_canvas_overlay(idx)
            self.save_selected_image(img_path, new_state)
            # 상태 업데이트
            selected_count = sum(1 for v in self.selected_images.values() if v)
            self.update_status_label(selected_count)
        else:
            logging.warning(f"Invalid selection index: {idx}")
    ...
    def save_selected_image(self, img_path, selected):
        """단일 이미지 저장/삭제"""
        ...
        if selected:
            ...
            try:
                if not os.path.exists(src_path):
                    logging.error(f"Source file {base_name} not found")
                    messagebox.showwarning("Warning", f"Source file {base_name} not found!")
                    return
                if os.path.exists(dst_path):
                    if not messagebox.askyesno("Overwrite", f"File {base_name} exists. Overwrite?"):
                        logging.info(f"Skipped overwriting {base_name}")
                        return
            ...
            try:
                ...
                if os.path.exists(crop_dst_path):
                    if not messagebox.askyesno("Overwrite", f"Crop file {crop_name} exists. Overwrite?"):
                        logging.info(f"Skipped overwriting {crop_name}")
                        return
            ...
            try:
                if os.path.exists(dst_path):
                    os.remove(dst_path)
                    logging.info(f"Deleted {base_name} from {self.save_folder}")
                if os.path.exists(crop_dst_path):
                    os.remove(crop_dst_path)
                    logging.info(f"Deleted cropped image {crop_name} from {self.save_folder}")
            except Exception as e:
                logging.error(f"Error deleting files for {base_name}: {e}")
                messagebox.showerror("Error", f"Failed to delete files for {base_name}: {e}")

    def update_status_label(self, selected_count):
        """상태 레이블 업데이트"""
        ...
        self.label_status.config(
            text=f"{start_idx + 1}/{len(self.image_files)} (남은 페이지: {remaining_pages}) | Selected: {selected_count}"
        )
