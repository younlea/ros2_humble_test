...
```
1. 필요한 라이브러리 설치:
pip install opencv-python-headless 그리고 GStreamer가 설치되어 있어야 합니다.
GStreamer 설치 방법은 OS에 따라 다르니, 설치를 진행하세요.
2. 송출기 실행: 첫 번째 프로그램을 실행하여 RTSP 스트림을 송출합니다.
3. 뷰어 실행: 두 번째 프로그램을 다른 IP에서 실행하고 <송출기의 IP>를 송출기의 실제 IP로 변경합니다.
4. 종료 방법: 송출기는 Ctrl+C, 뷰어는 ‘q’ 키로 종료할 수 있습니다.
