...
bool message__msg__data_ba__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  ...
  {  // databa
    PyObject * field = PyObject_GetAttrString(_pymsg, "databa");
    if (!field) {
      return false;
    }
    assert(PyLong_Check(field));
    ros_message->databa = (int32_t)PyLong_AsLong(field);
    Py_DECREF(field);
  }
...
PyObject * message__msg__data_ba__convert_to_py(void * raw_ros_message)
{
  ...
  {  // databa
    PyObject * field = NULL;
    field = PyLong_FromLong(ros_message->databa);
    {
      int rc = PyObject_SetAttrString(_pymessage, "databa", field);
      Py_DECREF(field);
      if (rc) {
        return NULL;
      }
